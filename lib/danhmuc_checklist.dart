...
class _DanhMuucCheckListState extends State<DanhMuucCheckList> {
  ...
  void initState() {
    ...
    _futureChecklist = _futureChecklist.then((list) {
      list.sort((a, b) {
        // Sort by active == 1 first, then by date descending
        int activeA =
            (a.active == '1' || a.active == 1 || a.active == true) ? 1 : 0;
        int activeB =
            (b.active == '1' || b.active == 1 || b.active == true) ? 1 : 0;
        if (activeA != activeB) {
          return activeB.compareTo(activeA); // active=1 first
        }
        return DateTime.parse(b.date)
            .compareTo(DateTime.parse(a.date)); // date desc
      });
      return list;
    });
...
Widget createViewItem(Checklist checklist, BuildContext context) {
  final bool isInactive = checklist.active == '0' ||
      checklist.active == 0 ||
      ...
      onTap: isInactive
          ...
          : () {
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      DanhMucLoaiMayScreen(checklist: checklist),
                ),
              );
            },
      child: Container(
        decoration: BoxDecoration(
          color: isInactive ? Colors.grey[300] : null,
          gradient: isInactive
              ? null
              : LinearGradient(
                  colors: [
        ...
        child: ListTile(
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                      fontSize: 15,
                      color: isInactive ? Colors.grey[600] : Colors.white70),
                ),
          ...
          trailing: Wrap(
            ...
            children: [
              IconButton(
                icon:
                    const Icon(Icons.home, color: Colors.greenAccent, size: 26),
                tooltip: 'Gửi về bờ',
                    ...
                    : () async {
                        ...
                        final result = await showDialog<DateTime>(
                          ...
                          builder: (ctx) {
                            ...
                            return AlertDialog(
                              title: const Text('Chọn ngày về bờ'),
                              content: SizedBox(
                                height: 200,
                        ...
                        if (result != null) {
                          ...
                          final success =
                              await setDateComeHome(checklist.id, result);
                          Navigator.of(context).pop(); // Đóng loading
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(success
                                  ? 'Đã gửi về bờ thành công!'
                                  : 'Gửi về bờ thất bại!'),
                              backgroundColor:
                    ...
                    : () async {
                        ...
                        if (confirm == true) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content: Text(
                                    'Deleted checklist: ${checklist.well}')),
                          );
                        }
                      },
              ),
...
/// Gửi ngày về bờ cho checklist
Future<bool> setDateComeHome(String checklistId, DateTime dateComeHome) async {
  final url = Uri.parse(
      'https://us-central1-checklist-447fd.cloudfunctions.net/setDateComeHome');
  try {
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        ...
        'date_come_home': dateComeHome.toIso8601String(),
      }),
    );
    if (response.statusCode == 200) {
      final jsonResponse = jsonDecode(response.body);
    ...
    print('Error setting date come home: $e');
    return false;
  }
}
